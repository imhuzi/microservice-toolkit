...
package com.uyibai.microservice.toolkit.generator.util;

import com.uyibai.microservice.toolkit.generator.annotation.ModelInterceptor;
...
public class ModelConverter {
    ...
    public static Map<String, String> getRequestBeanTypeMap(Class cls) {
        if (cls.isPrimitive()) {
            return null;
        }
        Method[] declaredMethods = cls.getDeclaredMethods();
        ...
        for (Method method : declaredMethods) {
            ...
            if (Modifier.isPublic(method.getModifiers()) && !Modifier.isStatic(method.getModifiers())
                    ...
                    && methodName.length() > 3 && Character.isUpperCase(method.getName().charAt(3))
                    && method.getParameterCount() == 1) {
                String name = StringUtils.uncapitalize(methodName.replace("set", ""));
                typeMaps.put(name, method.getGenericParameterTypes()[0].getTypeName());
            }
        }
        return typeMaps;
    }
