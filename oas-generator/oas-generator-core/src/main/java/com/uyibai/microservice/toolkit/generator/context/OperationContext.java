...
import com.uyibai.microservice.toolkit.generator.HttpStatuses;
import com.uyibai.microservice.toolkit.generator.MediaTypes;
import com.uyibai.microservice.toolkit.generator.parser.api.OpenApiAnnotationParser;
import com.uyibai.microservice.toolkit.generator.util.ModelConverter;
import com.uyibai.microservice.toolkit.generator.util.RequestResponse;
import com.uyibai.microservice.toolkit.oas.model.OasMethod;
import com.uyibai.microservice.toolkit.oas.model.OasParameter;
...
public class OperationContext implements IExtensionsContext {
    ...
    public OasMethod toOasMethod() {
        if (!hasOperation()) {
            return null;
        }

        OasMethod oasMethod = new OasMethod();
        oasMethod.setPath(parentContext.getBasePath() + getPath());
        oasMethod.setHttpMethod(getHttpMethod());

        if (StringUtils.isEmpty(operationId)) {
            operationId = method.getName();
        }
        oasMethod.setName(operationId);
        if (ArrayUtils.isNotEmpty(headers)) {
            oasMethod.setHeaders(new HashSet<>(Arrays.asList(headers)));
        }
        if (ArrayUtils.isNotEmpty(produces)){
            oasMethod.setProduces(new HashSet<>(Arrays.asList(produces)));
        }
        if (ArrayUtils.isNotEmpty(consumes)) {
            oasMethod.setConsumes(new HashSet<>(Arrays.asList(consumes)));
        }
        AtomicInteger indexAtom = new AtomicInteger(0);
        // 处理参数列表
        List<OasParameter> parameterList = parameterContexts.stream().map(parameterContext-> {
            parameterContext.setIndex(indexAtom.getAndIncrement());
            // 如果是 requestBody 类型的参数
            if (parameterContext.isRequestBody()) {
                OasParameter param = new OasParameter();
                param.setName(parameterContext.getName());
                param.setIn(parameterContext.getIn().name().toLowerCase());
                param.setRequired(parameterContext.getRequired());
                Schema schema = parameterContext.getSchema();
                if (schema == null) {
                    schema = new ObjectSchema();
                }
                param.setType(parameterContext.getRealType().getTypeName());
                param.setFormat(schema.getFormat());
                param.setRef(schema.get$ref());
                param.setIndex(parameterContext.getIndex());
                param.setMediaType(MediaTypes.APPLICATION_JSON);
                parameterContext.parserRealType(param);
                return param;
            }
            return parameterContext.toOasParameter();
        }).filter(Objects::nonNull).collect(Collectors.toList());
        oasMethod.setParameters(parameterList);
        oasMethod.setReturnType(method.getReturnType().getTypeName());
        return oasMethod;
    }
    ...
    public boolean isOneParam() {
        return parameterContexts != null && parameterContexts.size() == 1;
    }
